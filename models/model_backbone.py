...
class RoboVLMBackbone(nn.Module):
    def __init__(
        self,
        configs,
        train_setup_configs,
    ...
    def image_processor(self):
        ...
        image_preprocess = T.Compose([
            T.Resize(
                (
                    self.configs.get("image_size", 200),
                    self.configs.get("image_size", 200),
                ),
    ...
    def _trainable_parameters(self):
        ...
        if self.train_setup_configs["freeze_backbone"]:
            ...
            if self.train_setup_configs.get("train_decoder_layers", -1) == -1:
                ...
                model.requires_grad_(False)
                if hasattr(self.text_tower, "layers") and len(self.text_tower.layers) > 0:
                    for layer in self.text_tower.layers[:self.train_setup_configs.get("train_decoder_layers", -1)]:
    ...
    def forward_continuous(
        ...
        **kwargs,
        ...
        if vision_gripper is not None:
            multimodal_embeds, multimodal_labels, multimodal_attention_mask, _ = self.merge_multi_modal_input(
                multimodal_embeds,
                vision_gripper,
                mutlimodal_labels,
                multimodal_attention_mask,
